...
The [workflow](../.github/workflows/divergence.yml) runs on relevant working-branch pushes and manual dispatch, publishes both measurements in its job summary, and refreshes only the machine-marked sections of the registered issues. Human notes are preserved. It does not merge upstream, build/install binaries, or claim hardware acceptance. There is no scheduled refresh on the fork's default main branch: invoke the workflow on the documented working branch to refresh after upstream-only changes.
